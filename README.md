...
- Browser/PWA source candidate ปัจจุบันใช้ cache generation `kinaraidee-beta-v16`; deployment evidence ของ v16 ยังต้องยืนยันด้วย exact merged-main Pages + Live Smoke trace ก่อนถือว่า PASS
- v16 ยังคง atomic app-shell install ด้วย `cache.addAll(SHELL)`; ถ้า shell file ใดโหลดไม่สำเร็จ install จะไม่ถือว่าสมบูรณ์
- v16 เพิ่ม durable localStorage outbox สำหรับ member Favorite/Picked history, ป้องกัน stale cloud snapshot ทับ local writes และ retry pending writes หลัง restart/กลับมาออนไลน์; Issue #177 ยังต้อง post-v16 physical Android full-restart retest ก่อนปิด
- Latest verified deployed browser/PWA runtime ก่อน v16 คือ v15; ห้ามนำ v15 deployment/device evidence มาอ้างเป็น v16 PASS
...
- `ROLLBACK-RUNBOOK.md` — ขั้นตอน rollback สำหรับ release/PWA; เอกสารถูกเขียนแล้วแต่ยังต้องมี drill จริงก่อนถือว่าผ่าน
- `SECURITY.md` — Security gate, incident response และแนวทางรายงานช่องโหว่

หลักการสำคัญ: ห้ามเติมตัวเลข Beta, conversion หรือรายได้สมมติแทนข้อมูลที่วัดได้จริง และห้ามทำเครื่องหมาย Test Case, Live Deployment หรือ rollback drill ผ่านโดยไม่ได้ตรวจหลักฐานจริง
