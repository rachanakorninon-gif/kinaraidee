...
# Prototype source: keep providers outside Production runtime while making the staged
# client resilient to network/provider failures and programmatically readable.
p = Path('prototype/auth-multi-provider/member-auth-multi-provider.js')
text = p.read_text()

old = """  function providerLabel(provider){
...
new = """  function providerLabel(provider){
    if(provider===LINE_PROVIDER)return 'LINE';
    if(provider==='facebook')return 'Facebook';
    if(provider==='phone')return 'เบอร์โทร';
    if(provider==='email')return 'อีเมล';
  ...
  function createButton(text,className,enabled,onClick){"""
text = replace_once(text, old, new, 'insert safe redirect')

old = """    const setMessage=(text,ok=false)=>{
    ...
    let pendingPhone='';"""
new = """    const messageRegion=document.getElementById('msg');
    if(messageRegion){
    ...
    let pendingPhone='';"""
text = replace_once(text, old, new, 'message live region and redirect')

old = """    async function oauthLogin(provider,label){
...
new = """    function setProviderBusy(button,busy){
      if(!button)return;
    ...
    const lineBtn=createButton('LINE  ดำเนินการต่อด้วย LINE','authProvider authLine',rollout.line,()=>oauthLogin(LINE_PROVIDER,'LINE',lineBtn));"""
text = replace_once(text, old, new, 'oauth failure retry source')
...
text = replace_once(
    ...
    "const facebookBtn=createButton('f  ดำเนินการต่อด้วย Facebook','authProvider authFacebook',rollout.facebook,()=>oauthLogin('facebook','Facebook',facebookBtn));",
    'facebook busy binding',
)

old = """    sendPhoneOtp.addEventListener('click',async()=>{
...
new = """    sendPhoneOtp.addEventListener('click',async()=>{
      const phone=normalizeThaiPhone(phoneBox.querySelector('#authPhone').value);
      if(!phone)return setMessage('กรุณากรอกเบอร์มือถือไทยให้ถูกต้อง เช่น 0812345678');
      sendPhoneOtp.disabled=true;
...
text = replace_once(text, old, new, 'phone network recovery')

text = replace_once(
    text,
    "window.KINARAIDEE_MULTI_AUTH={init,normalizeThaiPhone,DEFAULT_ROLLOUT,LINE_PROVIDER};",
    "window.KINARAIDEE_MULTI_AUTH={init,normalizeThaiPhone,safeMemberRedirect,DEFAULT_ROLLOUT,LINE_PROVIDER};",
    'export redirect guard',
)
p.write_text(text)

# Strengthen the source-only guard without wiring any provider into Production.
p = Path('.github/workflows/auth-multi-provider-foundation-regression.yml')
text = p.read_text()
old = """          must(js.includes(\"signInWithOAuth({provider,options:{redirectTo}})\"),'social auth must use Supabase OAuth redirect flow');
          must(js.includes(\"signInWithOtp({phone})\"),'phone flow must use Supabase phone OTP');"""
new = """          must(js.includes(\"signInWithOAuth({provider,options:{redirectTo}})\"),'social auth must use Supabase OAuth redirect flow');
          must(js.includes('safeMemberRedirect(options.redirectTo)'),'OAuth callback must pass through the same-origin member redirect guard');
          must(js.includes('target.origin!==expected.origin||target.pathname!==expected.pathname'),'redirect guard must reject cross-origin or non-member destinations');
          must(js.includes(\"messageRegion.setAttribute('role','status')\")&&js.includes(\"messageRegion.setAttribute('aria-live','polite')\")&&js.includes(\"messageRegion.setAttribute('aria-atomic','true')\"),'provider feedback must have polite atomic status semantics');
          must(js.includes('setProviderBusy(button,true)')&&js.includes(\"button.setAttribute('aria-busy',busy?'true':'false')\"),'OAuth controls must expose a programmatic busy state');
          must(js.includes('ตรวจอินเทอร์เน็ตแล้วลองใหม่')&&js.includes('setProviderBusy(button,false)'),'network/provider failure must recover the button and present retry guidance');
          must(!js.includes('error.message'),'provider prototype must not surface raw provider/Auth error messages');
          must(js.includes(\"signInWithOtp({phone})\"),'phone flow must use Supabase phone OTP');"""
text = replace_once(text, old, new, 'regression auth accessibility checks')

old = """            echo '- Physical cross-account isolation, network/failure UX, accessibility, post-UI email/recovery regression and broader device coverage remain open before Production enablement.'
            echo '- Prototype client integration remains outside member.html and the Service Worker cache.'"""
new = """            echo '- Prototype source now guards same-origin member redirects, exposes polite status/busy semantics, and restores retry controls after caught OAuth/OTP network failures.'
            echo '- These are source-level preparations only; physical cross-account isolation, network/failure UX, accessibility, post-UI email/recovery regression and broader device coverage remain open before Production enablement.'
            echo '- Prototype client integration remains outside member.html and the Service Worker cache.'"""
text = replace_once(text, old, new, 'regression summary')
p.write_text(text)

# Record source-preparation truth while keeping the real rollout gates open.
...
# Final guardrails: no Production integration and no accidental rollout enablement.
