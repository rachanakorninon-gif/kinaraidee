...
Current Group API source candidate: PR #87 / `3b2375e50368add46e8b683111c30ed41be75715`, deployed as Supabase `group-api` ACTIVE version 5.
...
- run ID: `32631490603`
- exact `main` SHA: `524c185517b27c55c56218c8331b2a2ecec0f949`
- conclusion: `success`
- retained non-mutating rejection paths for unsupported method, malformed room IDs, malformed host-only token/id shapes and overlong voter ID
- additionally sent a >8 KiB body using HTTP/1.1 chunked transfer and verified HTTP 413 / `request_too_large`, exercising the actual-byte v5 guard instead of only the Content-Length early reject

Fresh Supabase Edge Function logs for the same controlled window show ACTIVE version 5 request-level platform entries with expected 405/400/403 status classes and POST 413 at `2026-08-23T09:37:00.760Z`. The inspected platform rows expose method, status, execution time, function/deployment identifiers and function version.
...
Execution times visible in the controlled v5 sequence are diagnostic observations only, **not** production traffic latency baseline, SLA, SLO, error budget or alert threshold.
...
- เมื่อมี body-limit change ให้แยก Content-Length early-reject evidence ออกจาก actual-body-byte evidence
