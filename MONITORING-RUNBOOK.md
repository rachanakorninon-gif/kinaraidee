...
This run remains historical v13 synthetic evidence. Current browser/PWA runtime is v16 and has its own Pages/Live Smoke evidence in `CURRENT-RUNTIME.md` / `CURRENT-RELEASE.md`. Historical synthetic success is not promoted into current-v16 real-device or Commercial acceptance.
...
- PR #220 adds a controlled `workflow_dispatch` alert-delivery self-test path. When `alert_self_test=true`, the workflow exits before any live Group API request is sent and uses a distinct `Group API alert delivery self-test` issue title so the test cannot be confused with a production incident.
...
Therefore Group evidence now proves source/deployment parity, rejection-only live behavior, recurring schedule configuration, real scheduled execution history, provider platform request-log visibility, and the existence of a controlled non-mutating alert self-test mechanism. **Actual alert delivery is still NOT VERIFIED** until a controlled self-test run is inspected together with the resulting GitHub issue/comment evidence. This evidence still does **not** prove exact `component=group-api` structured-event ingestion, production traffic baseline, SLA/SLO, monitoring owner/escalation, complete abuse control or approved retention.
...
- PR #221 adds a controlled `workflow_dispatch` alert-delivery self-test path. When `alert_self_test=true`, the workflow exits before any live Partner API request is sent and uses a distinct `Partner API alert delivery self-test` issue title so the test cannot be confused with a production incident.
...
Therefore Partner evidence now proves source/deployment hardening, rejection-only live behavior, recurring schedule configuration, real scheduled execution history, provider platform request-log visibility, and the existence of a controlled non-mutating alert self-test mechanism. **Actual alert delivery is still NOT VERIFIED** until a controlled self-test run is inspected together with the resulting GitHub issue/comment evidence. This evidence still does **not** prove successful search/click/conversion flows, real partner traffic, conversion/revenue, monitoring SLA, owner/escalation, complete anonymous rate/quota controls or approved retention.
...
สำหรับ controlled alert-delivery self-test:
- exact workflow run URL / run ID / repository SHA / trigger inputs
- resulting `Group API alert delivery self-test` หรือ `Partner API alert delivery self-test` issue/comment
- timestamp และ conclusion จริง
- ยืนยันว่า self-test exited before any live API request และไม่มี room/vote/partner application/click/conversion/payment/revenue action เกิดขึ้น
- ห้ามเปลี่ยนสถานะ actual alert delivery เป็น PASS หากยังไม่ได้ตรวจทั้ง workflow run และ resulting issue/comment จริง
...
การมี workflow, recurring probes, successful scheduled history, controlled alert self-test mechanism และ runbook นี้หมายถึง **monitoring mechanisms/evidence paths are functioning in the scoped synthetic sense** เท่านั้น. Operations monitoring gate ยังไม่ผ่านจนกว่าจะมีตาม Production scope ที่เปิดจริง:
...
- actual alert-delivery test with inspected resulting issue/comment evidence
...
ณ 2026-08-25 มี controlled alert-delivery self-test mechanisms สำหรับ Group API (PR #220) และ Partner API (PR #221) แล้ว แต่ **actual alert delivery ยัง NOT VERIFIED** เพราะยังไม่มี inspected controlled self-test run + resulting issue/comment evidence ใน canonical monitoring record. Scheduled rejection monitoring ที่สำเร็จจริงยังคงเป็นหลักฐานคนละชั้นและห้ามใช้แทน alert-delivery PASS.
