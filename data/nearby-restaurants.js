...
(function(){
  ...
  async function openMaps(lat,lon){const saved=await saveRequest(lat,lon);try{await api('track_search',{food:foodName(),source:'maps_fallback',lat,lon})}catch(e){}if(saved.ok)setStatus(saved.deduped?'เปิด Google Maps ให้แล้ว (คำขอนี้บันทึกไว้ก่อนหน้านี้แล้ว)':'บันทึกความต้องการเมนูนี้แล้ว กำลังเปิด Google Maps…');location.href=mapsUrl(foodName(),lat,lon)}
  ...
  async function loadPartners(lat,lon){try{const d=await api('find_partners',{food:foodName(),lat,lon,radiusKm:SEARCH_RADIUS_KM});renderPartners(d.partners||[]);setStatus(d.partners?.length?`พบร้านพาร์ตเนอร์ ${d.partners.length} ร้านสำหรับเมนูนี้`:'ยังไม่มีร้านพาร์ตเนอร์สำหรับเมนูนี้ — บันทึกความต้องการไว้แล้ว และค้นหาใน Google Maps ได้เลย')}catch(e){renderPartners([]);setStatus('ยังโหลดร้านพาร์ตเนอร์ไม่ได้ แต่คำขอยังบันทึกได้และค้นหาใน Google Maps ได้ตามปกติ',true)}}
  function useLocation(){if(!navigator.geolocation)return setStatus('อุปกรณ์นี้ไม่รองรับตำแหน่ง — เปิดค้นหาในแผนที่แทนได้ครับ',true);setBusy(true);setStatus('กำลังขอตำแหน่งของคุณ…');navigator.geolocation.getCurrentPosition(async p=>{lastCoords={lat:p.coords.latitude,lon:p.coords.longitude};const saved=await saveRequest(lastCoords.lat,lastCoords.lon);setStatus(saved.ok?(saved.deduped?'ตำแหน่งพร้อมแล้ว — กำลังหาร้านใกล้คุณ…':'✅ บันทึกคำขอแล้ว กำลังหาร้านพาร์ตเนอร์ใกล้คุณ…'):'ได้ตำแหน่งแล้ว แต่บันทึกคำขอไม่สำเร็จ — ยังลองค้นหาร้านได้ครับ',!saved.ok);await loadPartners(lastCoords.lat,lastCoords.lon);setBusy(false)},async()=>{const saved=await saveRequest();setStatus(saved.ok?'ไม่ได้รับสิทธิ์ตำแหน่ง แต่ ✅ บันทึกความต้องการเมนูนี้ไว้แล้ว':'ไม่ได้รับสิทธิ์ตำแหน่ง และยังบันทึกคำขอไม่ได้',!saved.ok);await loadPartners();setBusy(false)},{enableHighAccuracy:false,timeout:8000,maximumAge:300000})}
  function addUI(){if(document.getElementById('nearbyRestaurants'))return;const s=document.createElement('style');s.textContent='.nearbyHero,.nearbyCard,.fallbackCard{background:#fff;border:1px solid #eadfce;border-radius:18px;padding:14px;margin:10px 0}.nearbyStatus{border-radius:14px;padding:11px;margin:10px 0;font-weight:750}.nearbyMuted{font-size:14px;color:#6b5b4e;margin-top:5px}.privacyNote{font-size:12px;color:#786a60;text-align:center;margin:8px 12px 2px;line-height:1.5}.privacyNote a{color:#0d716c}.partnerBadge{display:inline-block;background:#edfaf8;color:#0d716c;padding:5px 9px;border-radius:99px;font-size:12px;font-weight:900}.partnerBtn{width:100%;border:0;border-radius:14px;padding:12px;margin-top:10px;font-weight:900;background:#ff8500;color:white}';document.head.appendChild(s);document.querySelector('main.app').insertAdjacentHTML('beforeend','<section id="nearbyRestaurants" class="screen"><div class="topbar"><button class="back" id="nearbyBack">‹</button><b>ร้านใกล้คุณ</b><span></span></div><div class="nearbyHero"><h2 id="nearbyTitle">หาร้านสำหรับเมนูนี้</h2><p>ใช้ตำแหน่งของคุณเพื่อค้นหาร้านใกล้เคียง</p></div><div id="nearbyStatus" class="nearbyStatus" style="display:none"></div><div id="nearbyPartners"></div><button class="primary" id="nearbyUseLocation">📍 ใช้ตำแหน่งปัจจุบัน</button><div class="privacyNote">ตำแหน่งจะถูกใช้เมื่อคุณกดปุ่มนี้เท่านั้น และพิกัดที่บันทึกเพื่อวิเคราะห์ความต้องการจะลดความละเอียดประมาณระดับย่าน • <a href="privacy.html">นโยบายความเป็นส่วนตัว</a></div><button class="secondary" id="nearbyOpenMaps">🗺️ ค้นหาใน Google Maps</button></section>');document.getElementById('nearbyBack').onclick=()=>show('result');document.getElementById('nearbyUseLocation').onclick=useLocation;document.getElementById('nearbyOpenMaps').onclick=()=>openMaps(lastCoords?.lat,lastCoords?.lon)}
  ...
  function openNearby(){try{targetFood=current}catch(e){targetFood=null}lastCoords=null;addUI();document.getElementById('nearbyTitle').textContent=foodName()?`หาร้าน “${foodName()}” ใกล้คุณ`:'หาร้านอาหารใกล้คุณ';document.getElementById('nearbyStatus').style.display='none';document.getElementById('nearbyPartners').replaceChildren();setBusy(false);show('nearbyRestaurants');loadPartners()}
  function init(){addUI();window.nearby=openNearby;window.KINARAIDEE_NEARBY={open:openNearby,openMaps};if(!window.KINARAIDEE_GROUP_MODE)loadScript('data/group-mode.js');loadScript('data/pwa-install.js')}
