...
- iPhone 17 Pro Max / iOS 26.6 มี scoped physical PASS เพิ่มสำหรับ installed Home Screen PWA ใน core recommendation/result interaction, Favorite/History local persistence และ Airplane Mode standalone reopen + cached core flow ตาม `IOS-PWA-PHYSICAL-EVIDENCE-20260905.md`; Member/Auth UI และ controlled sign-in ถูกสังเกตจริง แต่ sign-in → close → reopen session-persistence trace ยัง **INCONCLUSIVE** เพราะ successful sign-in กับ reopen ถูกจับคนละ browser context จึงยังไม่ใช่ Auth persistence PASS/FAIL และติดตามต่อใน Issue #534. หลักฐานนี้ไม่ทำให้ครบเกณฑ์ iPhone 2 distinct models โดยอัตโนมัติ
