...
- [x] OPPO Reno13 5G / Android 16 / installed standalone PWA, 2026-09-07: TC-02, TC-09, TC-10, TC-13, TC-14, NF-01, NF-02, NF-08 and NF-10 scoped physical PASS. Issue #551 Home bottom-navigation path has a later scoped **Physical PASS / CLOSED** on this affected device scope. Issue #552 / TC-03 remains **FAIL / OPEN** because the first remediation exposed a later false-positive (`ตำแตงไข่ต้ม` described as selected from `ซุป`); the merged follow-up fix is deployed in the current verified lineage, but fresh affected-device retest is still required before TC-03 Physical PASS. TC-08 allow remains **INCONCLUSIVE** because a fresh permission-grant prompt/state was not captured.
...
ขอบเขต: หลักฐานข้างต้นเป็น scoped physical sessions; exact model/OS/browser metadata บาง historical รายการไม่ได้ถูกบันทึกและห้ามเดา ห้ามขยาย scoped OPPO TC-11/TC-12/Auth/Reduced-Motion/NF-07/Product-Event/referral-summary PASS หรือ scoped Lenovo Keyboard Focus PASS ไปเป็น PASS ของรุ่นอื่น, referred-signup/referral-conversion, full device matrix, blanket Auth/Security PASS, real-user acquisition traction หรือ Commercial GO
