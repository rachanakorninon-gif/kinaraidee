...
- visible keyboard focus must still be verified with real keyboard/focus navigation on the current deployed runtime.
- reduced-motion behavior is **PASS for physical iPhone device/session #1 on the tested deployed v16 Surprise flow** (Issue #133, 2026-08-31). The scoped evidence covers Reduce Motion enabled, clear Home → busy → completed-result state changes, and no prolonged/non-essential visible motion in that tested flow. Exact iPhone model/iOS/browser-or-PWA versions were not captured and are not guessed. This does not establish Android reduced-motion coverage, second-iPhone coverage, keyboard-focus acceptance, NF-07, full device-matrix acceptance, Public Beta completion or Commercial GO.
...
- Physical iPhone/VoiceOver evidence closes NF-09 only for the tested session; scoped iPhone #1 Reduced Motion evidence closes only the tested reduced-motion acceptance path. Neither closes NF-07, second-device requirements, real form-submission acceptance, current Auth interaction acceptance, keyboard-focus acceptance, broader reduced-motion coverage or the remaining full-device-matrix gates.