...
- No assistant inference, example interval, historical timestamp range or schema default may populate an approval field.

## Required before Production retention can be described as approved

For each applicable data class, the canonical decision record must contain real reviewed values for its required owner/approver, retention period, scope/exceptions and Privacy/Terms reference. Any cleanup/anonymization mechanism then requires separate implementation, safe execution and post-run evidence.
...
This index asserts no user count, deletion count, retention duration, legal approval, partner readiness, conversion, revenue, test result, Public Beta completion or Commercial GO.
