...
Status: **FOUNDATION + CONTROLLED LINE CONFIG + ATTRIBUTION LIVE ACCEPTED / PRODUCTION PROVIDERS NOT ENABLED**
...
No provider is considered production-ready merely because provider configuration, server-side claim infrastructure, controlled attribution acceptance or UI/client code exists.

## Current production facts

- Supabase project: `cuspfvfzprlgtvtdyilh` (`Kinaraidee`).
- A controlled LINE Manual OAuth2 path is configured as `custom:line`; Production member UI remains email/password only.
...
- The reviewed server-side social/phone acquisition claim path from PR #531 is deployed as recorded by PR #532 / `SOCIAL-AUTH-ATTRIBUTION-DEPLOYMENT-EVIDENCE.md`: migration `20260905125841 / social_auth_attribution_claim_v1` is applied and Edge `member-acquisition-claim` is ACTIVE v1 with `verify_jwt=true`. Database EXECUTE remains restricted to privileged backend roles, and the reviewed RPC remains `SECURITY INVOKER` with empty `search_path`.
- Controlled attribution acceptance is now recorded separately in `SOCIAL-AUTH-ATTRIBUTION-LIVE-ACCEPTANCE-EVIDENCE.md`: fresh exactly-once LINE first-touch, returning-login immutability, forged/malformed request rejection, self/unresolved-referral negatives and the post-rollout email/password attribution regression are PASS within their recorded scope.
...
The provider-neutral post-auth claim path is deployed server-side and the controlled LINE attribution acceptance now proves, within its recorded scope, that it:
...
- preserves first-touch attribution and prevents duplicate first-touch state on repeated login;
- rejects forged/malformed/self-referral/unresolved-referral cases according to the reviewed contract;
- does not regress the existing email/password attribution path.

This acceptance closes the attribution-specific Issue #529 gates but is **not** Production provider rollout approval. New provider buttons must remain disabled until the remaining device/accessibility/account-isolation/failure gates close. Facebook and Phone OTP also require their own provider-specific configuration and acceptance rather than inheriting LINE evidence.
...
- existing email/password/recovery flow regression still passes after the provider UI is actually integrated;
- Public Beta and Commercial status remain separate gates.

Current controlled LINE evidence has established authorization reachability, cancel/return usability, one signed-out new LINE login, returning-user login to the same controlled LINE identity, user-initiated logout, and the attribution-specific acceptance recorded in `SOCIAL-AUTH-ATTRIBUTION-LIVE-ACCEPTANCE-EVIDENCE.md`. These scoped PASS items must not be generalized to the remaining gates or to Facebook/Phone.
