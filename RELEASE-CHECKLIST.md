...
- PR #520 is the post-acceptance referral-summary browser runtime. After a scoped signed-in physical session returned privacy-safe `EDGE` and satisfied the render acceptance checks, `member.html` removed the caller-scoped `get_my_referral_summary()` fallback and now uses JWT-verified `member-referral-api` as the only browser retrieval path. Optional QA trace now exposes only `EDGE/UNAVAILABLE`.
- Current verified browser/PWA deployment trace: merged-main descendant `aa470986589d83dd95b4efd6e4a4d68a9f55965d`; Pages run `33898258213` and main Live Smoke `33898314400` completed success on the traced PR #520 deployment/source. This is deployment/source evidence only; the separate physical referral-summary PASS is independently recorded and neither proves referral conversion, user growth or revenue.
...
- Historical PR #499 referral/acquisition runtime remains scoped deployment/acquisition evidence: source `f401ad758e40914a10245cfab08497f7cdb99f7d`, deployed descendant `02540bb61c3c62de4cfba34e92a876503765847d`, Pages `33811511793`, and Referral acquisition regression `33811512053`. This historical trace does not replace the current PR #520 runtime or establish a successful referred signup/referral conversion.
- Historical PR #373 Auth runtime remains scoped deployment/Auth evidence: source `6cd98bf2a2020b86fe2ab05e263dd59f7e4fb387`, deployed descendant `0cc3ec3ef4dda18f0d8e083d8ca0992ef77f844c`, Pages `33229525995`, Auth Password Security Live Smoke `33229548182`, and main Live Smoke `33229548190`. This historical trace does not replace the current PR #520 runtime.
...
- Current Auth account-flow evidence has scoped physical PASS on OPPO Reno13 5G / Android 16 / Chrome 152.0.7977.64 for recovery/password update/sign-in/new signup/email confirmation; leaked-password rejection remains NOT VERIFIED / blocked separately. This evidence is rooted in the PR #373 Auth flow but remains valid independently of PR #520 becoming the current browser runtime.
...
- [x] `CURRENT-RUNTIME.md` / `CURRENT-RELEASE.md` ระบุ current PR #520 runtime candidate และสถานะ **PASS FOR CURRENT BROWSER/PWA DEPLOYMENT TRACE** สอดคล้องกัน
- [x] Current PR #520 Member referral-summary Edge-only runtime มี canonical descendant Pages deployment + corresponding live-check evidence ตาม release contract — deployed descendant `aa470986589d83dd95b4efd6e4a4d68a9f55965d`, Pages `33898258213`, main Live Smoke `33898314400`; separate signed-in referral-summary physical `EDGE` acceptance และ post-cutover RPC privilege verification ถูกบันทึกไว้แยก
